...
export async function getForUser(userId: string): Promise<UserDetails | null> {
    ...
    if (!user) return null;
    return detailsRepo.getByUserId(userId);
}
...
export async function createForUser(userId: string, dto: CreateUserDetailsDto): Promise<UserDetails> {
    ...
    // Map undefined -> null to comply with the repository (string | null)
    const payload = {
    ...
    return detailsRepo.create(userId, payload);
}
...
export async function patchForUser(userId: string, dto: PatchUserDetailsDto): Promise<UserDetails> {
    ...
    // Only include the keys present in the DTO.
    // If the key is present, map undefined -> null; if not present, leave untouched.
    ...
    return detailsRepo.update(userId, data);
}
